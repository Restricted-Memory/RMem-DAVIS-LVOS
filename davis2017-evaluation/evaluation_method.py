...
default_davis_path = '/public/home/zhoujunbao/datasets/DAVIS'
# default_davis_path = '/public/home/zhoujunbao/datasets/long_videos'
...
if os.path.exists(csv_name_global_path) and os.path.exists(csv_name_per_sequence_path) and not args.re:
    ...
    g_measures = ['J&F-Mean', 'J-Mean', 'J-Recall', 'J-Decay', 'F-Mean', 'F-Recall', 'F-Decay', 'J_last-Mean', 'J_last-Recall', 'J_last-Decay']
    final_mean = (np.mean(J["M"]) + np.mean(F["M"])) / 2.
    g_res = np.array([final_mean, np.mean(J["M"]), np.mean(J["R"]), np.mean(J["D"]), np.mean(F["M"]), np.mean(F["R"]),
                      np.mean(F["D"]), np.mean(J_last["M"]), np.mean(J_last["R"]), np.mean(J_last["D"])])
    ...
    seq_measures = ['Sequence', 'J-Mean', 'F-Mean', 'J_last-Mean']
    J_per_object = [J['M_per_object'][x] for x in seq_names]
    F_per_object = [F['M_per_object'][x] for x in seq_names]
    J_last_per_object = [J_last['M_per_object'][x] for x in seq_names]
    table_seq = pd.DataFrame(data=list(zip(seq_names, J_per_object, F_per_object, J_last_per_object)), columns=seq_measures)
