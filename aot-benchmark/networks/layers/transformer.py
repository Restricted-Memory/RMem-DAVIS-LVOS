...
class LongShortTermTransformerBlock(nn.Module):
    ...
    def forward(self,
        ...
        if temporal_pos is not None:
            bs, embed_dim, h, w = local_K.shape
            token_num = h * w
            T = global_K.shape[0] // token_num

            cur_pos_emb, mem_pos_emb = temporal_pos[0:1], temporal_pos[1:]
            ...
            if T == 1:
                global_K_with_pe = global_K + mem_pos_emb[0].view(1, 1, embed_dim)
            else:
                ...
                if T <= max_T:
                    ...
                    # print(f"interpolate nearest")
                    interpolated_mem_pe = F.interpolate(interpolated_mem_pe, size=max_T, mode='linear', align_corners=True)
                    interpolated_mem_pe = torch.flip(interpolated_mem_pe, dims=(-1,))
                    interpolated_mem_pe = F.interpolate(interpolated_mem_pe, size=T, mode='nearest')
                    interpolated_mem_pe = torch.flip(interpolated_mem_pe, dims=(-1,))

                interpolated_mem_pe = interpolated_mem_pe.view(embed_dim, T).permute(1, 0).contiguous()

                global_K_with_pe = global_K.view(T, token_num, bs, embed_dim) + interpolated_mem_pe.view(T, 1, 1, embed_dim)
                global_K_with_pe = global_K_with_pe.view(T * token_num, bs, embed_dim).contiguous()

            curr_Q_with_pe = curr_Q + cur_pos_emb.view(1, 1, embed_dim)
            tgt2, attn = self.long_term_attn(curr_Q_with_pe, global_K_with_pe, global_V, is_return_attn_weight=save_atten_weights)
        else:
            tgt2, attn = self.long_term_attn(curr_Q, global_K, global_V, is_return_attn_weight=save_atten_weights)
        ...
        if temporal_pos is not None:
            ...
            if T == 1:
                ...
                if T <= max_T:
                    ...
                    interpolated_mem_pe = F.interpolate(interpolated_mem_pe, size=T, mode='nearest')
                    interpolated_mem_pe = torch.flip(interpolated_mem_pe, dims=(-1,))
                    # print(f"nearest flip : {interpolated_mem_pe[0, 0, :] = }")
            ...
            curr_Q_with_pe = curr_Q + cur_pos_emb.view(1, 1, embed_dim)
            cat_tgt2, attn = self.long_term_attn(curr_Q_with_pe, global_K_with_pe, cat_global_V,
                                            cat_curr_U, size_2d, is_return_attn_weight=save_atten_weights)
        else:
            cat_tgt2, attn = self.long_term_attn(curr_Q, global_K, cat_global_V,
                                            cat_curr_U, size_2d, is_return_attn_weight=save_atten_weights)
        if save_atten_weights:
            bs, head, hw, thw = attn.size()
            self.record_T = thw // hw
