...
class AOT(nn.Module):
    def __init__(self, cfg, encoder='mobilenetv2', decoder='fpn'):
        ...
        self.use_temporal_pos_emb = cfg.USE_TEMPORAL_POS_EMB
        if cfg.USE_TEMPORAL_POS_EMB:
            self.cur_pos_emb = nn.Parameter(torch.randn(1, cfg.MODEL_ENCODER_EMBEDDING_DIM) * 0.02)
            self.mem_pos_emb = nn.Parameter(torch.randn(4, cfg.MODEL_ENCODER_EMBEDDING_DIM) * 0.02)
            trunc_normal_(self.cur_pos_emb, std=.02)
            trunc_normal_(self.mem_pos_emb, std=.02)
    ...
    def LSTT_forward(self,
        ...
        lstt_embs, lstt_memories = self.LSTT(curr_emb, long_term_memories,
                                             short_term_memories, curr_id_emb=curr_id_emb,
                                             self_pos=pos_emb, temporal_pos=temporal_pos_emb, size_2d=size_2d,
