...
class AOTEngine(nn.Module):
    ...
    def add_reference_frame(self,
        ...
        if self.AOT.use_temporal_pos_emb:
    ...
    def update_long_term_memory(self, new_long_term_memories):
        ...
        self.long_memories_indexes.append(self.frame_step)
        print(f"{self.long_memories_indexes = }")
        if self.long_term_memories is None:
        ...
        # hw = self.short_term_memories[0][0].size(-2) * self.short_term_memories[0][0].size(-1)
        # t = updated_long_term_memories[0][0].size(0) / hw
        if self.cfg.RESTRIC_MEMORY and \
            (updated_long_term_memories[0][0].size(0) > (self.cfg.FORMER_MEM_LEN + self.cfg.LATTER_MEM_LEN)):
            ...
            # print(f"Restric Memory")
            former_memory_len = self.cfg.FORMER_MEM_LEN
            latter_memory_len = self.cfg.LATTER_MEM_LEN
            hw, b, c = new_e.size()
            to_drop_idx = former_memory_len
            # to_drop_idx = random.randint(former_memory_len, latter_memory_len)

            if self.cfg.RESTRIC_MEMORY_USING_ATTEN_WEIGHT:
                attn_weight = torch.stack([
                    self.AOT.LSTT.layers[0].record_attn_weight,
                    self.AOT.LSTT.layers[0].record_attn_weight,
                    # self.AOT.LSTT.layers[1].record_attn_weight,
                    # self.AOT.LSTT.layers[2].record_attn_weight,
                ]).mean(dim=0)
                ...
                # moving mean
                attn_weight_dict = {
                    self.long_memories_indexes[i]: attn
                ...
                for i, _ in enumerate(attn_weight):
                    attn_weight[i] = attn_weight_dict[self.long_memories_indexes[i]]
                print(f"{attn_weight = }")

                # UCB
                frame_times = {
                    mem_idx: 1
                    for mem_idx in self.long_memories_indexes
                }
                ...
                frame_times_np = torch.Tensor([
                    frame_times[mem_idx]
                    for mem_idx in self.long_memories_indexes[:-1]
                ])
            ...
            print(f"{to_drop_idx = }")
            is_drop = False
            for layer_idx in range(len(updated_long_term_memories)):
                memory_k_v = updated_long_term_memories[layer_idx]
                # print(f"{layer_idx = }")
                for i in range(len(memory_k_v)):
                    ...
                    thw, b, c = mem.size()
                    t = thw // hw
                    mem_t = mem.view((t, hw, b, c))
                    # print(f"{mem_t.size() = }")
                    if mem_t.size(0) > (former_memory_len + latter_memory_len):
                        is_drop = True
                        new_mem = torch.cat([mem_t[0:to_drop_idx, ...], mem_t[to_drop_idx+1:, ...]], dim=0)
                        # print(f"{new_mem.size() = }")
                        updated_long_term_memories[layer_idx][i] = new_mem.view(((former_memory_len+latter_memory_len)*hw, b, c))
            if is_drop:
                self.long_memories_indexes.remove(self.long_memories_indexes[to_drop_idx])
        self.long_term_memories = updated_long_term_memories
        hw = self.short_term_memories[0][0].size(-2) * self.short_term_memories[0][0].size(-1)
        t = self.long_term_memories[0][0].size(0) / hw
        print(f"{t = }")
    ...
    def match_propogate_one_frame(self, img=None, img_embs=None):
        ...
        if self.AOT.use_temporal_pos_emb:
            temporal_pos_emb = torch.cat((self.AOT.cur_pos_emb, self.AOT.mem_pos_emb), dim=0) 
            if (self.frame_step % self.long_term_mem_gap) == 0:
                print(f"{temporal_pos_emb.size() = }")
        else:
            temporal_pos_emb = None
        self.curr_lstt_output = self.AOT.LSTT_forward(curr_enc_embs,
                                                      ...
                                                      temporal_pos_emb=temporal_pos_emb,
                                                      size_2d=self.enc_size_2d,
                                                      save_atten_weights=self.cfg.RESTRIC_MEMORY_USING_ATTEN_WEIGHT,
