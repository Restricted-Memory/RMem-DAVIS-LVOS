...
from dataloaders.train_datasets import DAVIS2017_Train, YOUTUBEVOS_Train, StaticTrain, TEST, LVOS_Train
    ...
    def process_pretrained_model(self):
        ...
        if cfg.TRAIN_RESUME:
            if self.rank == 0:
                try:
                    try:
                        ...
                        print(f"Attempting to load from {ema_ckpt_dir = }")
                        ema_model, removed_dict = load_network(
                            self.model, ema_ckpt_dir, self.gpu)
                        print(f"Load from {ema_ckpt_dir = } complete !")
                    except Exception as inst:
                        self.print_log(inst)
            ...
            try:
                ...
                print(f"Attempting to load from {resume_ckpt = }")
                self.model, self.optimizer, removed_dict = load_network_and_optimizer(
                    ...
                    self.gpu,
                    scaler=self.scaler)
                print(f"Load from {resume_ckpt = } complete !")
            except Exception as inst:
                self.print_log(inst)
            ...
            if cfg.PRETRAIN_FULL:
                try:
                    print(f"Attempting to load from {cfg.PRETRAIN_MODEL = }")
                ...
                print(f"Attempting to load from {cfg.MODEL_ENCODER_PRETRAIN = }")
    ...
    def prepare_dataset(self):
        ...
        if 'lvos' in cfg.DATASETS:
            train_lvos_dataset = LVOS_Train(
                ...
                transform=composed_transforms,
                seq_len=cfg.DATA_SEQ_LEN,
                rand_gap=cfg.DATA_RANDOM_GAP_LVOS,
                rand_reverse=cfg.DATA_RANDOM_REVERSE_SEQ,
                merge_prob=cfg.DATA_DYNAMIC_MERGE_PROB,
                enable_prev_frame=self.enable_prev_frame,
                max_obj_n=cfg.MODEL_MAX_OBJ_NUM)
            train_datasets.append(train_lvos_dataset)
