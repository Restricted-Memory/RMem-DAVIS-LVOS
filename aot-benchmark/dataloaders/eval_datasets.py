...
class LVOS_Test(DAVIS_Test):
    def __init__(self,
        ...
        if split[0] == 'val':
            ...
            split = split[0]
        self.image_root = os.path.join(root, split, split, 'JPEGImages')
        self.label_root = os.path.join(root, split, split, 'Annotations')
        seq_names = os.listdir(self.label_root)
        self.seqs = seq_names

    def __getitem__(self, idx):
        seq_dataset = super().__getitem__(idx)
        seq_dataset.resolution = None
        return seq_dataset
